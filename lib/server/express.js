...
// parse application/json
  ...
  _server.use(xmlparser({normalize:true,normalizeTags:true}))
  ...
  console.log("_app.config.cookie",_app.config.cookie)
  ...
  _app.routes.register(_app, new Restify(_server, _app));

  if (_app.routes_dir != undefined) {
    for (var _r in _app.routes_dir) {
      if (_app.routes_dir[_r].register != undefined) {
        _app.routes_dir[_r].register(_app, new Restify(_server, _app));
      } else {
        console.error("Route [" + _r + "] register function is missed!");
      }
    }
  }
